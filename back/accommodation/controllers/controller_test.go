...
func TestGetAccomodationHandler_Success(t *testing.T) {
	// Create a new Gin router
	router := gin.Default()

	// Create a mock service that returns a list of accomodations
	...
	router.GET("/accomodations/:location", func(c *gin.Context) {
		c.Set("services.GetAccomodations", mockService)
		GetAccomodationHandler(c)
	})

	// Create a test HTTP request to GET the accomodations for "Paris"
	req, _ := http.NewRequest("GET", "/accomodations/Paris", nil)
...
func TestGetAccomodationHandler_Error(t *testing.T) {
	// Create a new Gin router
	router := gin.Default()
	...
	// Create a test HTTP request to GET the accomodations for "IUYT6FGO6UQGFUYEGBLOIUY7RGFOUYHDSVBCUOYTESQFV"
	req, _ := http.NewRequest("GET", "/accomodations/IUYT6FGO6UQGFUYEGBLOIUY7RGFOUYHDSVBCUOYTESQFV", nil)
