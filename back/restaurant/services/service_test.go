...
func TestGetRestaurants(t *testing.T) {
	...
	restaurants, err := services.GetRestaurants(location)

	// Check the results
	assert.NoError(t, err)
	assert.NotEmpty(t, 1, len(restaurants))
	assert.NotEmpty(t, restaurants[0].Name)
	assert.NotEmpty(t, restaurants[0].Address)
	assert.NotEmpty(t, restaurants[0].Location)
	assert.NotEmpty(t, restaurants[0].PriceLevel)
	assert.NotEmpty(t, restaurants[0].Photo)
}

func TestGetrestaurantsummary(t *testing.T) {
	// Set up mock server
	...
	// Call the function being tested
	restaurant, err := services.Getrestaurantsummary(placeID)

	// Check the results
	assert.NoError(t, err)
	assert.Empty(t, restaurant.Description)
}

func TestMapPlacesSearchResultTorestaurant(t *testing.T) {
	// Set up test data
	...
	// Call the function being tested
	restaurant := services.MapPlacesSearchResultTorestaurant(place)

	// Check the results
	assert.NotEmpty(t, restaurant.PlaceID)
	assert.NotEmpty(t, restaurant.Name)
	assert.NotEmpty(t, restaurant.Address)
	assert.NotEmpty(t, restaurant.PriceLevel)
	assert.NotEmpty(t, restaurant.Photo)
}
